...
#        shared.log.debug(" data= %s" % data)
#        shared.log.debug(" fmt = '%s'" % fmt)
#        shared.log.debug("toto = bitstring.pack(fmt, **data)")
#        pack = bitstring.pack(fmt, **data)
#        shared.log.debug("DEBUG -- bitstring.pack(fmt, **data): %s" % pack)
#        pack_byte = pack.tobytes()
#        shared.log.debug("DEBUG -- pack_byte: %s" % pack_byte)
...
class DmxPacket(ArtNetPacket):
    ...
    def __init__(self, frame=None, **kwargs):
        ...
        self.frame = frame or dmx_frame.Frame()
    
    @classmethod
    def parse_framedata(cls, b, fmt):
        return dmx_frame.Frame([ord(x) for x in b.read('bytes:512')])
...
#        shared.log.debug(len(tmp))
#        tmp = tmp[0:512]
#        shared.log.debug(len(tmp))
#        tmp2 = str.encode(tmp)
#        shared.log.debug(len(tmp2))
...
class PollPacket(ArtNetPacket):
    opcode = dmx_definitions.OPCODES['OpPoll']
    schema = (
        ('header', 'bytes:8'),
        ('opcode', 'int:16'),
        ('protocol_version', 'uintbe:16'),
...
class PollReplyPacket(ArtNetPacket):
    opcode = dmx_definitions.OPCODES['OpPollReply']
    counter = 0
    
    port = dmx_definitions.STANDARD_PORT
    
    short_name = b'ArtNet Lighting Controller'
    long_name = b'https://github.com/nbremond77/Artnet_dmx_controler'
    ...
    def format_node_report(self):
        tmp = "#0001 [%s] Power On Tests successful" % PollReplyPacket.counter
        node_report = str.encode(tmp)
...
class TodRequestPacket(ArtNetPacket):
    opcode = dmx_definitions.OPCODES['OpTodRequest']
    schema = (
        ('header', 'bytes:8'),
        ('opcode', 'int:16'),
        ('protocol_version', 'uintbe:16'),
